...
def test_whatsapp_enquiry_accepts_full_contact_details():
    async def exercise():
        payload = LeadCreate(
            first_name="Ada",
            last_name="Lovelace",
        ...
        assert inserted["last_name"] == "Lovelace"
        assert inserted["phone"] == "+18765550123"
        assert lead is not None

    asyncio.run(exercise())


def test_whatsapp_enquiry_requires_last_name():
    async def exercise():
        payload = LeadCreate(
            first_name="Ada",
            email="ada@example.com",
...
def test_whatsapp_enquiry_requires_phone():
    async def exercise():
        payload = LeadCreate(
            first_name="Ada",
            last_name="Lovelace",
