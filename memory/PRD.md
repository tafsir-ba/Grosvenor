...
- **Amenities empty space:** category grids switched from fixed 3-col grid (left an orphan 6th cell with 5 items) to `flex flex-wrap` with `flex-1 basis` so each category renders a clean 3 + 2 (bottom row fills full width). FOLLOW-UP FIX: dividers now use per-card `border-r border-b` (+ container `border-l border-t`) instead of flex `gap-px`, so a divider line renders between EVERY adjacent pair including the two bottom-row cards. Verified by testing_agent (iteration_6.json, 100% pass).
